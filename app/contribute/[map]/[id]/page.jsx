...
// helper to sanitize HTML and rewrite external links via /link?url=...
...
export default async function Location({ params, searchParams }) {
  ...
  if (!location) {
    // You might want a nicer 404 here
  ...
  if (location.coordinates && location.coordinates.includes(",")) {
  ...
  console.log(
    "open",
    `/${map}?locked=1&lng=${panX}&lat=${panY}&name=${encodeURIComponent(
      location.name
    )}&type=${location.geometry}&search=0&hamburger=0`
  )
    ...
    <div className="mx-auto my-4 flex justify-center flex-col md:container select-text">
      ...
      <Card className="">
        <CardHeader>
          ...
          <span className="">
            Coordinates:{" "}
            <span className="text-gray-400">{coordPretty}</span>
          </span>
          {location.faction && (
            <span className="inline">
              Faction:{" "}
              <span className="text-gray-400 inline">{location.faction}</span>
            </span>
          )}
          {location.city && (
            <span className="inline">
              City:{" "}
              <span className="text-gray-400 inline">{location.city}</span>
            </span>
          )}
          {location.alias && (
            <span className="inline">
              Alias:{" "}
              <span className="text-gray-400 inline">{location.alias}</span>
            </span>
          )}
          {location.source && (
            <span className="">
              Source:{" "}
              <span className="text-gray-400">{location.source}</span>
            </span>
          )}
        </CardHeader>
              ...
              {isNaN(Number(panX)) || isNaN(Number(panY)) ? (
                ...
                <iframe
                  src={`/${map}?locked=1&lng=${panX}&lat=${panY}&z=${
                    ...
                    location.name
                  )}&type=${location.geometry}&search=0&hamburger=0&zoom=0`}
                  width="600"
                  height="400"
                  style={{ border: "none" }}
                ></iframe>
