...
export default function DrawerComponent({ drawerContent, setDrawerContent, IS_GALAXY, coordinates, GENERATE_LOCATIONS, name, height, selectedId, mobile, d, myGroup }) {
  ...
  if (GENERATE_LOCATIONS) {
    local = generateLocations(myGroup, coordinates)
  }
      ...
      <DrawerContent>
          ...
          <DrawerTitle className="text-center" asChild>
            <div>
              ...
              <div className="flex justify-center mt-2">
                <Link href={genLink(d, name, "href")} className="block w-6 opacity-60" target={`${name === "lancer" ? "_self" : "_blank"}`}>
                  <LinkIcon className="cursor-pointer" size={mobile ? 14 : 19} />
                </Link>
                {d.properties.name} <span className=" text-gray-400">- {d.properties.type}</span>
              </div>
            </div>
          </DrawerTitle>
        </DrawerHeader>
        ...
        <div className="min-w-64 p-2">
          <div className="flex justify-center w-full">
            <img src={`${svgBase + name + "/" + d.properties.type + '.svg'}`} alt={d.properties.type} width="70px" height="70px" align="left" />
          </div>
