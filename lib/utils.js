...
export const svgBase =
  "https://raw.githubusercontent.com/CodaBool/stargazer/refs/heads/main/public/svg/"
export const MENU_HEIGHT_PX = 40
export const USER_LOCATION_ID_START = 1073741824
...
export const AVAILABLE_PROPERTIES = {
  ...
  description:
    "text which describes the feature and its significance|type=text",
  ...
  faction:
    "the faction this feature aligns with, typically used with territories to change the color|type=comma list",
  ...
  stroke:
    "border color for polygons and lines. Point locations do not use stroke.|type=rgba",
}
...
export function createPopupHTML(e, map) {
  ...
  let badges = [
    e.features[0].properties.unofficial &&
      '<div class="inline-flex text-white items-center rounded-full border px-2.5 py-0.5 text-xs font-bold transition-colors focus:outline-hidden focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 dark:border-slate-800 dark:focus:ring-slate-300 border-transparent bg-red-900 hover:bg-red-900/80 mx-auto my-1">unofficial</div>',
    e.features[0].properties.destroyed &&
      '<div class="inline-flex text-black items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-hidden focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 dark:border-slate-800 dark:focus:ring-slate-300 border-transparent bg-slate-100 hover:bg-slate-100/80 mx-auto my-1">destroyed</div>',
    e.features[0].properties.capital &&
  ...
  const url = icon ? icon : `${svgBase}${map}/${type}.svg`
  ...
  return `
    <div class="min-w-64 p-2">
      ...
      <div style="width:70px;height:70px;float:right;padding-left:10px">
        <img src="${url}" alt="${type}" width="70px" height="70px" align="left">
      </div>
      <hr class="my-3"/>
...
const forceAttrs = (svg, fill, stroke) => {
  if (fill) {
    svg = svg.replace(
      /(<(path|circle|rect|polygon|g)[^>]*?)\s*(fill=".*?")?/gi,
      (match, before) => {
  ...
  if (stroke) {
    svg = svg.replace(
      /(<(path|circle|rect|polygon|g)[^>]*?)\s*(stroke=".*?")?/gi,
      (match, before) => {
  ...
  if (d?.properties?.icon) {
    ...
    if (d.properties.icon.startsWith("http")) {
      ...
      if (type) {
        ...
        console.log(
          `WARN: failed to create relative icon path: ${d.properties}. Type ${t}`,
        )
        return null
      }
    }
...
export function getConsts(map) {
  const DEFAULTS = {
    ...
    STYLE: {
      ...
      sources: {},
      sprite:
        "https://raw.githubusercontent.com/CodaBool/stargazer/refs/heads/main/public/svg/spritesheet",
  ...
  if (map === "fallout") {
    ...
    return {
      ...
      TRAVEL_RATE: 0.995,
      QUOTE:
        "...along ancient charted paths and out toward new worlds. Union could not bring their dead back home. But they would choke the stars with the living",
    ...
    return {
      ...
      GENERATE_LOCATIONS: false,
      TRAVEL_RATE: 5,
      // BG_IMAGE: "/background/mothership.webp",
      BG_IMAGE: "https://images.codabool.workers.dev/img/mothership.webp",
      // DISTANCE_CONVERTER: 0.047496521, // arbitrary number to convert km to parsec
      QUOTE: "Survive, Solve, Save. Pick 1",
      // UNIMPORTANT: ["star", "terrestrial", "station", "jovian"],
      // UNIMPORTANT_ZOOM: [8.5, 9],
      // UNIT: "parsec",
      // TRAVEL_RATE: 16,
    ...
    return {
      ...
      SEARCH_SIZE: 0.03,
      GENERATE_LOCATIONS: false,
      DISTANCE_CONVERTER: 8.827169407, // km to mi in the geojson area I have
      UNIT: "mi",
      IS_GALAXY: false,
      TRAVEL_RATE_UNIT: "mph",
      TRAVEL_RATE: 3, // average walking speed in miles per hour
      // BG_IMAGE: "/background/dnd.jpg",
      BG_IMAGE: "https://images.codabool.workers.dev/img/dnd.webp",
      ...
      VIEW: {
        ...
        // bgImage: [[.4429, -.311], [.516, -.265]],
        // ORIGINAL = bgImage: [[.445, -.311], [.513, -.265]]
      },
      MAX_ZOOM: 11,
      MIN_ZOOM: 8.71,
    }
...
export function normalizeFeatures(features, allKeys) {
  ...
  return features.map(feature => {
    // avoid mutating callers if you care (recommended)
    const f = {
      ...feature,
    ...
    allKeys.forEach(propKey => {
      if (!Object.prototype.hasOwnProperty.call(f.properties, propKey)) {
        f.properties[propKey] = null
      }
    })

    if (typeof f.properties.FID !== "undefined" && f.properties.FID !== null) {
      f.properties.FID = String(f.properties.FID)
    }
...
export function combineLayers(geojsons) {
  const allKeys = collectAllPropertyKeys(geojsons)

  let combinedFeatures = []
  geojsons.forEach(geojson => {
    if (geojson?.features) {
      combinedFeatures = combinedFeatures.concat(
        normalizeFeatures(geojson.features, allKeys),
      )
    }
  })
...
// Function to normalize properties
export function combineLayersForTopoJSON(geojsons) {
  ...
  geojsons.forEach(geojson => {
    if (geojson?.features) {
      ...
      normalized.forEach(feature => {
        ...
        } else if (geomType.includes("Poly")) {
  ...
  return {
    location: {
      type: "FeatureCollection",
      features: categorizedFeatures.location,
    },
    territory: {
      type: "FeatureCollection",
      features: categorizedFeatures.territory,
    },
...
export function combineAndDownload(type, serverTopojson, clientGeojson) {
  try {
    ...
    const serverGeojsonLocation = feature(
      serverTopojson,
      serverTopojson.objects?.["location"] || {
        type: "GeometryCollection",
        geometries: [],
      },
    )
    const serverGeojsonTerritory = feature(
      serverTopojson,
      serverTopojson.objects?.["territory"] || {
        type: "GeometryCollection",
        geometries: [],
      },
    )
    const serverGeojsonGuide = feature(
      serverTopojson,
      serverTopojson.objects?.["guide"] || {
        type: "GeometryCollection",
        geometries: [],
      },
    )
    const serverGeojsonLabel = feature(
      serverTopojson,
      serverTopojson.objects?.["label"] || {
        type: "GeometryCollection",
        geometries: [],
      },
    )
...
export function collectAllPropertyKeys(geojsons) {
  const allKeys = new Set()
  geojsons.forEach(geojson => {
    geojson?.features?.forEach(f => {
      Object.keys(f.properties ?? {}).forEach(key => allKeys.add(key))
...
export function combineLayersDedup(geojsons, opts = {}) {
  const {
    keyFn = defaultFeatureKey,
    lastWriteWins = true,
  ...
  // normalize everything first (shared logic)
  const normalizedLists = geojsons
    .filter(g => g?.features?.length)
    .map(g => normalizeFeatures(g.features, allKeys))

  const map = new Map()

  const add = f => {
    let key = keyFn(f)
    if (!key && geometryFallback) key = geometrySignature(f)
    if (!key) {
      // no stable key: keep it unique by reference so it doesn't get dropped
      key = `__noid__:${cryptoLikeStableStringify(f)}`
    }
    ...
    if (lastWriteWins) {
      ...
      // first write wins: do nothing
    }
  }

  if (lastWriteWins) {
    // earlier then later; later overwrites
    normalizedLists.flat().forEach(add)
  } else {
    // keep first occurrence
    normalizedLists.flat().forEach(f => {
      const k = String(
        keyFn(f) ?? (geometryFallback ? geometrySignature(f) : ""),
      )
      if (!k || !map.has(k)) add(f)
    })
  }
...
function defaultFeatureKey(f) {
  return f?.id ?? f?.properties?.id ?? f?.properties?.FID ?? null
}
function geometrySignature(f) {
  const t = f?.geometry?.type ?? ""
  const c = f?.geometry?.coordinates ?? null
  return `${t}:${JSON.stringify(c)}`
}
function cryptoLikeStableStringify(obj) {
  return JSON.stringify(obj, Object.keys(obj).sort())
}

export function genLink(d, map, type) {
  if (!d) return null
  // TODO: there is an id system, I should find a way to make direct contribute links
  if (map.includes("lancer")) {
    ...
    return type === "href"
      ? `https://lancer.wiki.gg/wiki/Special:Search?search=${encodeURIComponent(d.properties.name)}`
      : "_blank"
  } else if (map === "fallout") {
    return type === "href"
      ? `https://fallout.fandom.com/wiki/Special:Search?query=${encodeURIComponent(d.properties.name)}`
      : "_blank"
  } else if (map === "starwars") {
    return type === "href"
      ? `http://starwars.wikia.com/wiki/Special:Search?query=${encodeURIComponent(d.properties.name)}`
      : "_blank"
  } else if (map === "warhammer") {
    return type === "href"
      ? `https://warhammer40k.fandom.com/wiki/Special:Search?query=${encodeURIComponent(d.properties.name)}`
      : "_blank"
  } else if (map === "alien") {
    // There are two fandom sites for Alien, avp is slightly better
    return type === "href"
      ? `https://avp.fandom.com/wiki/Special:Search?query=${encodeURIComponent(d.properties.name)}`
      : "_blank"
  } else if (map === "cyberpunk") {
    return type === "href"
      ? `https://cyberpunk.fandom.com/wiki/Special:Search?query=${encodeURIComponent(d.properties.name)}`
      : "_blank"
  } else if (map === "dnd") {
    return type === "href"
      ? `https://forgottenrealms.fandom.com/wiki/Special:Search?query=${encodeURIComponent(d.properties.name)}`
      : "_blank"
  }
}
...
export function getPaint(map, geometry, layer, STYLES) {
  ...
  if (geometry === "fill") {
    ...
    return {
      ...
      "line-width": [
        "case",
        ["in", ["get", "type"], ["literal", ["highway", "replace_me"]]],
        3.5,
        2.5,
      ],
      // 'line-dasharray': [6, 1],
    ...
    const defaults = {
      "text-color": ["coalesce", ["get", "text-color"], "#fff"],
      "text-opacity": [
        "interpolate",
        ["linear"],
        ["zoom"],
      ...
      "icon-opacity": ["coalesce", ["get", "icon-opacity"], 1],
      "icon-color": [
        "case",
        ["boolean", ["feature-state", "hover"], false],
...
export function getColorExpression(map, style, geo) {
  if (map === "fallout") {
    if (style === "stroke") {
      if (geo === "Point") {
        ...
        return ["coalesce", ["get", style], "rgb(139, 178, 141)"]
      } else {
        return [
          "coalesce",
          ["get", style],
          [
            "case",
            ["==", ["get", "faction"], "Brotherhood of Steel"],
            "rgba(39, 122, 245, 0.1)",
            ["==", ["get", "faction"], "Ceasar's Legion"],
            "rgba(245, 81, 39, 0.2)",
            ["==", ["get", "faction"], "NCR"],
            "rgba(133, 92, 0, .5)",
            ["==", ["get", "type"], "region"],
    ...
    if (style === "stroke") {
      return [
        ...
        [
          ...
          ["==", ["get", "type"], "cluster"],
          "rgba(39, 83, 245, 0.6)",
          ["==", ["get", "name"], "Karrakin Trade Baronies"],
          "rgba(247, 173, 77, 1)",
          ["==", ["get", "name"], "Harrison Armory"],
          "rgba(99, 0, 128, 1)",
          ["==", ["get", "name"], "IPS-N"],
          "rgba(158, 0, 0, 1)",
          ["==", ["get", "faction"], "interest"],
          "rgba(84, 153, 199, .6)",
          ["==", ["get", "name"], "Union Coreworlds"],
          "rgba(245, 39, 39, 0.6)",
          ["==", ["get", "type"], "territory"],
      ...
      if (geo === "Point") {
        ...
        return [
          ...
          [
            ...
            ["==", ["get", "faction"], "union"],
            "rgba(227, 49, 5, 0.45)",
            ["==", ["get", "name"], "The Interest"],
            "rgba(84, 153, 199, .6)",
            ["==", ["get", "name"], "The Long Rim"],
            "rgba(84, 153, 199, .6)",
            ["==", ["get", "type"], "gate"],
            "teal",
            ["==", ["get", "type"], "star"],
    ...
    if (style === "stroke") {
      ...
      return [
        ...
        [
          ...
          ["==", ["get", "faction"], "ICSC"],
          "rgba(18, 138, 3, 0.2)", // green
          ["==", ["get", "faction"], "3WE"],
          "rgba(232, 179, 46, 0.2)", // orange
          ["==", ["get", "faction"], "UPP"],
          "rgba(232, 46, 46, 0.2)", // red
          ["==", ["get", "faction"], "UA"],
          "rgba(46, 46, 232, 0.2)", // blue
          ["==", ["get", "type"], "terrestrial"],
    ...
    if (style === "stroke") {
      if (geo === "LineString") {
        ...
        return [
          ...
          [
            ...
            ["==", ["get", "type"], "bg"],
            "black", // blue
            ["==", ["get", "threat"], "corporate"],
            "rgba(46, 46, 232, 0.2)", // blue
            ["==", ["get", "threat"], "suburbs"],
            "rgba(232, 179, 46, 0.2)", // orange
            ["==", ["get", "threat"], "executive"],
            "rgba(115, 16, 142, 0.2)", // purple
            ["==", ["get", "threat"], "combat"],
            "rgba(232, 46, 46, 0.2)", // red
            ["==", ["get", "threat"], "extreme"],
            "rgba(232, 46, 46, 0.5)", // red
            ["==", ["get", "type"], "apartment"],
            "rgba(39, 122, 245, 1)",
      ...
      return [
        ...
        [
          ...
          ["==", ["get", "type"], "bg"],
          "black", // blue
          ["==", ["get", "type"], "bg-radiated"],
          "green", // blue
          ["==", ["get", "name"], "NorCal Military Base"],
          "rgba(232, 46, 46, .1)", // red, looks good
          ["==", ["get", "name"], "Watson Development"],
          "rgba(46, 46, 232, .1)", // blue, looks good
          ["==", ["get", "name"], "New Westbrook North"],
          "rgba(232, 179, 46, .1)", // yellow
          ["==", ["get", "name"], "Little Europe"],
          "rgba(255, 0, 197, 0.07)", // purple, looks good
          ["==", ["get", "name"], "Port of Night City"],
          "rgba(0, 197, 255, 0.1)", // blue
          ["==", ["get", "name"], "Reclamation Zone"],
          "rgba(0, 197, 255, 0.1)", // blue
          ["==", ["get", "name"], "Santo Domingo"],
          "rgba(0, 197, 255, 0.1)", // blue
          ["==", ["get", "name"], "North Heywood"],
          "rgba(245, 95, 39, 0.1)", // orange
          ["==", ["get", "name"], "Heywood Docks"],
          "rgba(0, 10, 255, 0.15)", // blue
          ["==", ["get", "name"], "New Westbrook"],
          "rgba(0, 197, 255, 0.1)", // blue
          ["==", ["get", "name"], "Exec Zone"],
          "rgba(245, 39, 219, 0.08)", // pink
          ["==", ["get", "name"], "Charter Hill"],
          "rgba(146, 39, 245, 0.14)", // purple
          ["==", ["get", "name"], "Upper Marina"],
          "rgba(0, 197, 255, 0.1)", // blue
          ["==", ["get", "name"], "Downtown"],
          "rgba(0, 197, 255, 0.1)", // blue
          ["==", ["get", "name"], "Kabuki"],
          "rgba(108, 245, 39, 0.08)", // green
          ["==", ["get", "name"], "Old Japantown"],
          "rgba(0, 197, 255, 0.1)", // blue
          ["==", ["get", "name"], "The Hot Zone"],
          "rgba(0, 0, 0, 1)", // black, looks good
          ["==", ["get", "name"], "Morro Rock"],
          "rgba(0, 0, 0, 1)", // black, looks good
          ["==", ["get", "name"], "University District"],
          "rgba(16, 255, 0, 0.06)", // green, looks good
          ["==", ["get", "name"], "South Night City"],
          "rgba(255, 149, 0, 0.08)", // orange
          ["==", ["get", "name"], "The Glen"],
          "rgba(143, 143, 255,.08)", // blue
          ["==", ["get", "name"], "Old Combat Zone"],
          "rgba(255, 143, 143, .12)", // red
          ["==", ["get", "name"], "Old Japan Town"],
          "rgba(143, 255, 153, .1)", // green
          ["==", ["get", "name"], "Little China"],
          "rgba(255, 0, 0, 0.08)", // red
          ["==", ["get", "name"], "Morro Rock"],
          "rgba(255, 255, 255, .05)", // grey
          ["==", ["get", "name"], "Heywood"],
          "rgba(250, 255, 0, 0.12)", // yellow
          ["==", ["get", "name"], "Pacifica Playground"],
          "rgba(0, 255, 218, 0.09)", // cyan
          ["==", ["get", "name"], "Rancho Coronado"],
          "rgba(255, 0, 212, 0.07)", // purple
          ["==", ["get", "name"], "Heywood Industrial Zone"],
          "rgba(255, 255, 255, .1)", //
          ["==", ["get", "name"], "New Westbrook South"],
          "rgba(255, 0, 0, 0.04)", // red
          ["==", ["get", "name"], "Executive Zone"],
          "rgba(90, 0, 255, 0.08)", // purple
      ...
      if (geo === "LineString") {
        ...
        return [
          ...
          [
            ...
            ["==", ["get", "faction"], "empire"],
            "rgba(38, 113, 188, 0.2)",
            ["==", ["get", "faction"], "alliance"],
            "rgba(125, 0, 0, .2)",
            ["==", ["get", "faction"], "neutral/hutt"],
            "rgba(50, 135, 44, 0.2)",
            ["==", ["get", "faction"], "the chiss ascendancy"],
            "rgba(0, 244, 255, 0.2)",
            ["==", ["get", "type"], "line"],
            "rgba(0, 0, 0, 0)",
            "rgba(126, 126, 100, 0.2)", // default color
    ...
    if (style === "stroke") {
      if (geo === "LineString") {
        return ["coalesce", ["get", "stroke"], "rgb(139, 178, 141)"]
      } else {
        return [
          "coalesce",
          ["get", "stroke"],
      ...
      return [
        ...
        [
          "case",
          ["==", ["get", "faction"], "Adeptus Astartes/Space Marines"],
          "#3783FF", // blue
          ["==", ["get", "faction"], "Adeptus Mechanicus"],
          "#F60000", // red
          ["==", ["get", "faction"], "Adeptus Administratum"],
          "#FF8C00", // orange
          ["==", ["get", "faction"], "asuryani"],
          "#45D0D0", // cyan
          ["==", ["get", "faction"], "chaos"],
          "#F60000", // red
          ["==", ["get", "faction"], "daemon"],
          ...
          ["==", ["get", "faction"], "Daemon"],
          "#F60000", // red
          ["==", ["get", "faction"], "Dark Mechanicum"],
          "#F60000", // red
          ["==", ["get", "faction"], "drukhari"],
          "#F60000", // red
          ["==", ["get", "faction"], "Empire of the Severed"],
          "#17630d", // dark green
          ["==", ["get", "faction"], "Exodite"],
          "#45D0D0", // cyan
          ["==", ["get", "faction"], "Genestealer Cult"],
          "#ca1c8a", // pink
          ["==", ["get", "faction"], "Ghulo Industrial Complex"],
          "#ffffff", // white
          ["==", ["get", "faction"], "Khorne"],
          "#F60000", // red
          ["==", ["get", "faction"], "Kronus Hegemony"],
          "#ffffff", // white
          ["==", ["get", "faction"], "leagues-of-votann"],
          "#ffffff", // white
          ["==", ["get", "faction"], "Sons of Malice"],
          "#ffffff", // white
          ["==", ["get", "faction"], "Mephrit Dynasty"],
          "#17630d", // dark green
          ["==", ["get", "faction"], "necron"],
          "#1c8b0e", // monster green
          ["==", ["get", "faction"], "Nihilakh Dynasty"],
          "#1c8b0e", // monster green
          ["==", ["get", "faction"], "Nurgle"],
          "#52661a", // puke green
          ["==", ["get", "faction"], "Ogdobekh Dynasty"],
          "#5c5c5c", // grey
          ["==", ["get", "faction"], "ork"],
          "#799a1d", // ork smash
          ["==", ["get", "faction"], "Void pirates"],
          "#ffffff", // white
          ["==", ["get", "faction"], "Roguetrader"],
          "#FF8C00", // orange
          ["==", ["get", "faction"], "Sarnekh Dynasty"],
          "#541566", // dark purple
          ["==", ["get", "faction"], "Sautekh Dynasty"],
          "#1c8b0e", // monster green
          ["==", ["get", "faction"], "Adepta Sororitas"],
          "#FF8C00", // orange
          ["==", ["get", "faction"], "tau"],
          "#FF8C00", // orange
          ["==", ["get", "faction"], "Thokt Dynasty"],
          "#ffffff", // white
          ["==", ["get", "faction"], "tyranid"],
          "#73108e", // purple
          ["==", ["get", "faction"], "Tzeentch"],
          "#7984be", // baby blue
          ["==", ["get", "faction"], "Urani-Surtr Regulates"],
          "#ffffff", // white
          ["==", ["get", "faction"], "Ymyr Conglomerate"],
          "#ffffff", // white
          ["==", ["get", "faction"], "imperial"],
          "#fff1ab", // light gold
          ["==", ["get", "faction"], "unclassified"],
          "#949494", // light grey
          ["==", ["get", "type"], "anomaly"],
          "#ffffff", // white
          ["==", ["get", "type"], "asteroid"],
  ...
  console.log(
    "ERR: getColorExpression error, likely new map and needs conditionals added",
  )
}
...
export function getDailyMenuQuote() {
  const menuQuotes = [
    ...
    "All models become obsolete eventually.", // Commentary on tech lifespan
  ...
  setTimeout(() => {
    ...
    if (window.WebGLRenderingContext) {
      ...
      try {
        ...
        console.log("ERR: WebGL is supported, but disabled")
      }
    }
    ...
    if (!hasHWA) {
      ...
      window.alert("No WebGL support found. Most features will not work.")
    }
  }, 6_000)
}
...
function swRoundedColFromLon(lonDeg) {
  ...
  const offset = Math.round(
    lonDeg / SW_STEP + (lonDeg === 0 ? 0 : Math.sign(lonDeg) * 1e-9),
  )
...
export function gridHelpers(name, GRID_DENSITY) {
  ...
  function axisLabel(deg, alignment) {
    if (name === "alien") {
      const idx = Math.round(
        deg * (1 / GRID_DENSITY) + (deg === 0 ? 0 : Math.sign(deg) * EPS),
      )
  ...
  function hasGrid(name) {
    return name === "alien" || name === "starwars"
  }
  ...
  function coordFromLngLat(lon, lat, decimal = 1) {
    ...
    const roundedLabel =
      name === "starwars" ? `${rCol}-${rRow}` : `${rCol},${rRow}`
    let cellLabel = name === "starwars" ? `${cCol}-${cRow}` : `${cCol},${cRow}`

    // should use lat, lng for other maps
    if (!hasGrid(name)) {
      cellLabel = `${cRow},${cCol}`
    }
  ...
  return { formatLabels, coordFromLngLat, hasGrid }
}
...
export function withUserFilters(baseFilter, userFilterExpr) {
  if (!userFilterExpr || userFilterExpr === true) return baseFilter ?? true
  if (!baseFilter || baseFilter === true) return userFilterExpr
  const baseIsAll = Array.isArray(baseFilter) && baseFilter[0] === "all"
  const userIsAll = Array.isArray(userFilterExpr) && userFilterExpr[0] === "all"
  if (baseIsAll && userIsAll)
    return ["all", ...baseFilter.slice(1), ...userFilterExpr.slice(1)]
  if (baseIsAll) return ["all", ...baseFilter.slice(1), userFilterExpr]
  if (userIsAll) return ["all", baseFilter, ...userFilterExpr.slice(1)]
  return ["all", baseFilter, userFilterExpr]
}

export function hasFilterClause(filter, clause) {
  if (!Array.isArray(filter)) return false
  if (
    filter.length === clause.length &&
    filter.every((v, i) => v === clause[i])
  )
    return true
  for (const part of filter) {
    if (Array.isArray(part) && hasFilterClause(part, clause)) return true
  }
  return false
}
...
export function sanitizeContent(html, sanitizeFunc) {
  ...
  return sanitizeFunc(html, {
    allowedTags: [
      ...
      "ul", "ol", "li", "blockquote", "code", "pre", "a", "h1", "h2", "h3", "h4", "h5", "h6"
    ],
    allowedAttributes: {
      a: ["href", "target", "rel"],
    },
    ...
    transformTags: {
      a: (tagName, attribs) => {
        ...
        // Optionally rewrite untrusted outbound links
        if (
          href &&
          !href.startsWith("/") &&
          !href.startsWith("https://stargazer.vercel.app/") &&
          !href.startsWith(process.env.NEXT_PUBLIC_URL)
        ) {
...
export function boundsToCoord(maxBounds) {
  if (
    !Array.isArray(maxBounds) ||
    maxBounds.length !== 2 ||
    !Array.isArray(maxBounds[0]) ||
    !Array.isArray(maxBounds[1])
  ) {
    throw new Error("Invalid maxBounds format")
  }

  const [[west, south], [east, north]] = maxBounds

  if ([west, south, east, north].some(v => typeof v !== "number")) {
    throw new Error("maxBounds values must be numbers")
  }
