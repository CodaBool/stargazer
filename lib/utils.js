...
export function getConsts(map) {
  ...
  if (map === "fallout") {
    ...
    return {
      ...
      //   "icon-size": .7,
      // },
      // Alien Stars of the Middle Heavens is 47 parsec x 30 parsec
...
export function genLink(d, map, type) {
  ...
  if (map.includes("lancer")) {
    ...
    return type === "href" ? `https://alienanthology.fandom.com/wiki/Special:Search?query=${encodeURIComponent(d.properties.name)}` : "_blank"
  }
}
