...
export const AVAILABLE_PROPERTIES = {
  "type": "dual purpose, a category for the feature but will also be used as an SVG<br/> if matching the name of <a style='color: rgb(133, 163, 255);' target='_blank' href='https://github.com/CodaBool/stargazer.vercel.app/wiki/Sci%E2%80%90Fi-maps-for-all#icon'>available icons</a> and no icon key is present|required|type=text",
  ...
  "fill": "interior color for polygons. Also works as the color for markers. Lines don't use fill.|type=rgba",
  "stroke": "border color for polygons and lines. markers do not use stroke.|type=rgba",
}
...
export function createPopupHTML(e, isGalaxy) {
  ...
  const svgBase = "https://raw.githubusercontent.com/CodaBool/stargazer/refs/heads/main/public/svg/" + (isGalaxy ? "lancer/" : "utils.jsL91/")
  ...
  return `
    <div class="min-w-64">
      ...
      <div style="width:80px;height:80px;float:right;padding-left:10px">
        <img src="${svgBase + type + '.svg'}" alt="${type}" width="80px" height="80px" align="left">
      </div>
      <hr class="my-3"/>
      <p>${description}</p>
