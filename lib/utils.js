...
export const AVAILABLE_PROPERTIES = {
  ...
  "faction": "the faction this feature aligns with, typically used with territories to change the color|type=comma list",
...
export function createPopupHTML(e, map) {
  ...
  let badges = [
    e.features[0].properties.unofficial && '<div class="inline-flex text-white items-center rounded-full border px-2.5 py-0.5 text-xs font-bold transition-colors focus:outline-hidden focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 dark:border-slate-800 dark:focus:ring-slate-300 border-transparent bg-red-900 hover:bg-red-900/80 mx-auto my-1">unofficial</div>',
...
export function getConsts(map) {
  const DEFAULTS = {
    ...
    TYPES: {
      ...
      "point": ["station","ecumenopolis", "jovian", "moon", "terrestrial", "desert_planet", "ocean_planet", "barren_planet", "ice_planet", "asteroid", "lava_planet", "ringed_planet", "gate", "black_hole", "wormhole", "exoplanet", "ship", "star", "neutron_star", "comet", "unknown"],
  ...
  if (map === "fallout") {
    ...
    return {
      ...
      VIEW: {
        ...
        zoom: 8,
        // [[left, bottom], [right, top]]
        maxBounds: [[-1.3734, -0.8696], [1.37315, 0.86925]],
...
export function combineLayersDedup(geojsons, opts = {}) {
  ...
  if (lastWriteWins) {
    ...
    normalizedLists.flat().forEach((f) => {
      const k = String(keyFn(f) ?? (geometryFallback ? geometrySignature(f) : ""));
...
export function genLink(d, map, type) {
  ...
  if (map.includes("lancer")) {
    ...
    return type === "href" ? `https://forgottenrealms.fandom.com/wiki/Special:Search?query=${encodeURIComponent(d.properties.name)}` : "_blank"
  }
}
...
export function hardwareCheck() {
  ...
  setTimeout(() => {
    ...
    if (!hasHWA) {
      ...
      window.alert(
        "No WebGL support found. Most features will not work."
      )
    }
  }, 6_000)
}
...
export function withUserFilters(baseFilter, userFilterExpr) {
  ...
  if (baseIsAll && userIsAll) return ["all", ...baseFilter.slice(1), ...userFilterExpr.slice(1)];
...
export function boundsToCoord(maxBounds) {
  ...
  if (
    [west, south, east, north].some(v => typeof v !== "number")
  ) {
