...
export function createPopupHTML(e, isGalaxy, map, setDrawerContent) {
  ...
  const badges = [
    ...
    e.features[0].properties.faction && `<div class="inline-flex text-black items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-hidden focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 dark:border-slate-800 dark:focus:ring-slate-300 border-transparent bg-white hover:bg-blue-600/80 mx-auto my-1">${e.features[0].properties.faction}</div>`,
  ...
  return `
    <div class="min-w-64 p-2">
      ...
      <div class="flex flex-col items-center mt-1.5">${badges}</div>
    </div>
  `
}
...
export function getConsts(map) {
  ...
  if (map === "fallout") {
    ...
    return {
      ...
      TYPES: {
        "polygon": ["territory", "zone", "threat", "lake", "ocean"],
        "point": ["apartment",
          "arts",
          "bar",
          "braindance",
    ...
    if (style === "stroke") {
      if (geo === "LineString") {
        return [
          ...
          [
            "case",
            ['==', ['get', 'type'], 'highway'], '#f3e600',
            ['==', ['get', 'type'], 'road'], '#c5003c',
            ["==", ["get", "name"], "filler"], "rgba(117, 186, 143, 0.1)", // green
            "rgba(117, 186, 143, 0.08)" // default green
          ]
        ];
...
// Where the numbering for rows starts (top edge):
// 6.5° latitude -> row 0, then +1 for each 0.65° downward
const SW_TOP_LAT = 6.5;

// Which letter sits on the prime meridian (0° lon):
// ... -0.65=K, 0=L, +0.65=M, etc
const SW_ZERO_LON_LETTER = "L";
const SW_STEP = 0.65;

// ------- helpers -------
const fixNegZero = (n) => (Object.is(n, -0) ? 0 : n);

// Excel-style 1..∞ -> A, B, ... Z, AA, AB, ...
// We'll adapt it for 0-based indices.
function indexToLetters(index) {
  // clamp to 0 to avoid going before 'A'
...
function letterToIndex(letter) {
  // 'A'->0, 'B'->1, ..., 'Z'->25
  // supports multi-letter like 'AA'
  const up = letter.toUpperCase();
  let n = 0;
  for (let i = 0; i < up.length; i++) {
    n = n * 26 + (up.charCodeAt(i) - 65 + 1);
  }
  return n - 1; // back to 0-based
}

// Column label from longitude (letters).
// 0° -> 'L', -0.65 -> 'K', -6.5 -> 'B', -7.15 -> 'A', +0.65 -> 'M', ...
function longitudeToColumnLetter(lonDeg) {
  ...
  const colIndex = baseIndex + offset;
  return indexToLetters(colIndex);
}

// Row number from latitude (numbers).
// 6.5 -> 0, 5.85 -> 1, ... 0 -> 10, -0.65 -> 11, ...
function latitudeToRowNumber(latDeg) {
  const rowsDown = Math.round((SW_TOP_LAT - latDeg) / SW_STEP + 1e-9);
  return rowsDown;
}

export function gridAlgorithm(deg, alignment) {
  if (alignment) {
    // horizontal: package passes longitude for meridian labels
    const lon = fixNegZero(+deg.toFixed(10));
    return longitudeToColumnLetter(lon);
  } else {
    // vertical: package passes latitude for parallel labels
    const lat = fixNegZero(+deg.toFixed(10));
    return String(latitudeToRowNumber(lat));
  }
}
